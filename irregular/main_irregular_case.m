...
%% read data
load irartiData(16x16)_L; %column-major index
Xtrain=XDat(:,1:256);
Ltttrain=Ltt(1:256);
Lngtrain=Lng(1:256);
[n,p]=size(Xtrain);
...
[Jp,tri]=Del_triangle(Ltttrain,Lngtrain);
...
Gtest=Prm_interpolation(Ltttest,Lngtest,Ltttrain,Lngtrain,tri,Gh,alpha_g); %only num of rows needed due to the col-major index
Stest=Prm_interpolation(Ltttest,Lngtest,Ltttrain,Lngtrain,tri,Sh,alpha_s);
Ltest=Prm_interpolation(Ltttest,Lngtest,Ltttrain,Lngtrain,tri,Lh,alpha_u);

%% interpolating waveheights in the latent Gaussian layer using MIDW
GXtest = Val_interpolation(Ltttest,Lngtest,Ltttrain,Lngtrain,tri,GXtrain,K);
