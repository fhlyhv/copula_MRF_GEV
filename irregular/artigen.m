...
s2=16;  %num of cols
nobs=256;
nuobs= 400; %number of unobserved random sites
...
Ltt=rand(nobs,1)*(s1-1)+1;
Lng=rand(nobs,1)*(s2-1)+1;
% Ltt(1:8)=ones(1,8);
% Ltt(9:16)=s1*ones(1,8);
% Lng(17:24)=ones(1,8);
% Lng(25:32)=s2*ones(1,8);
Ltt=[Ltt;rand(nuobs,1)*(s1-3)+2];
Lng=[Lng;rand(nuobs,1)*(s2-3)+2];
...
%     XDat=gevrnd(G*ones(nsample,p),S*ones(nsample,p),repmat(L',nsample,1));
    save irartiData(16x16)_L XDat Lng Ltt;
...
% elseif model==2
    ...
    save irartiData(16x16)_SL XDat Lng Ltt;
